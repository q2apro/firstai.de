...
public class XML2iPhone {
	...
	public static void main(String[] args) throws Exception {
		...
		for (String lang : args) {
			...
			try (BufferedReader br = new BufferedReader(new InputStreamReader(XML2iPhone.class.getResourceAsStream("/iphone-template.txt"), StandardCharsets.UTF_8));
					BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("../../iphone-html-version/firstaid" + lang.toUpperCase() + ".html"), StandardCharsets.UTF_8))) {
				String line;
				while ((line = br.readLine()) != null) {
					if (line.equals("${MENU}")) {
						for (Map.Entry<String, String> entry : language.getMenuEntries().entrySet()) {
							bw.write("        <li><a href=\"#" + entry.getValue() + "\">" + h(entry.getKey()) + "</a></li>\n");
						}
					} else if (line.equals("${DISCLAIMERAUDIO}")) {
						if (!language.getString("audioPrefix").isEmpty())
							bw.write("<a href=\"audio/" + h(language.getString("audioPrefix")) + "/0.mp3\" target=\"_blank\" type=\"audio/mpeg\" class=\"playbtn\">&nbsp;Play</a><br /><br/>\n");
						...
						for (String id : language.getSectionIDs()) {
							...
							for (Node n = section.getFirstChild(); n != null; n = n.getNextSibling()) {
								...
								if (elem.getNodeName().equals("title")) {
									bw.write("<ul id=\"" + id + "\" title=\"" + h(elem.getTextContent()) + "\"> ");
									if (!section.getAttribute("headImage").isEmpty())
										bw.write("<img src=\"images/aid" + String.format("%03d", Integer.parseInt(section.getAttribute("headImage"))) + ".png\" />");
								} else if (elem.getNodeName().equals("altTitle")) {
									// handled for the menu already
								} else if (elem.getNodeName().equals("audio")) {
									if (!language.getString("audioPrefix").isEmpty())
										bw.write("\n<a href=\"audio/" + language.getString("audioPrefix") + "/" + elem.getTextContent() + "\" target=\"_blank\" type=\"audio/mpeg\" class=\"playbtn\">&nbsp;Play</a><br /><br/>");
									...
									if (filename.equals("aid111.png"))
										filename = "aid111.jpg";
									bw.write(" <img src=\"images/" + filename + "\" />");
									...
									for (Node sub = elem.getFirstChild(); sub != null; sub = sub.getNextSibling()) {
										...
										bw.write("<a href=\"#" + se.getAttribute("target") + "\">" + h(se.getTextContent()) + "</a>");
									}
								}
							}
							bw.write("</li></ol></ul>\n");
						...
						for (String sc : sortedContinents) {
							bw.write("\n  <li><a target=\"_self\" href=\"#tel" + sc + "\">" + h(language.getString("continent_" + sc)) + "</a></li>");
						}
						...
						for (String continent : sosNumbers.getContinents()) {
							...
							for (String c : sortedCountries) {
								allCountries.add(c);
								bw.write("\t<li><a href=\"#" + c + "\">" + h(language.getString("country_" + c)) + "</a></li>\n");
							}
							bw.write("\t</ul>\n");
						}
						bw.write("<!-- ALL COUNTRIES WITH EMERGENCY NUMBERS -->\n");
						for (String cc : allCountries) {
							String[] numbers = sosNumbers.getNumbers(cc);
							bw.write("<ul id=\"" + cc + "\" title=\"" + h(language.getString("country_" + cc)) + "\"><br /><ol>");
