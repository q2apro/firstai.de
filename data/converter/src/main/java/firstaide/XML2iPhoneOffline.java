...
/**
 * Generate iPhone Offline HTML files from XML files.
 */
public class XML2iPhoneOffline {
	...
	public static void main(String[] args) throws Exception {
		...
		for (String lang : args) {
			...
			try (BufferedReader br = new BufferedReader(new InputStreamReader(XML2iPhone.class.getResourceAsStream("/iphone-offline-template.txt"), StandardCharsets.UTF_8));
					BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("../../iphone-html-version/firstaid" + lang.toUpperCase() + "offline.html"), StandardCharsets.UTF_8))) {
				String line;
							...
							for (Node n = section.getFirstChild(); n != null; n = n.getNextSibling()) {
								...
								if (elem.getNodeName().equals("title")) {
									...
									if (!section.getAttribute("headImage").isEmpty())
										bw.write("<img src=\"" + dataURL("aid" + String.format("%03d", Integer.parseInt(section.getAttribute("headImage"))) + ".png") + "\" />");
									...
									if (filename.equals("aid111.png"))
										filename = "aid111.jpg";
									bw.write(" <img src=\"" + dataURL(filename) + "\" />");
	...
	private static String dataURL(String filename) throws IOException {
		BufferedImage img = ImageIO.read(new File("../../iphone-html-version/images/" + filename));
		BufferedImage img2 = new BufferedImage(img.getWidth() / 2, img.getHeight() / 2, img.getType());
